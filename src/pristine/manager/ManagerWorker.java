...
public class ManagerWorker extends CDAPCallbackInterface implements Runnable{
	...
	static String IPCP_2 = "/computingSystemID=1/processingSystemID=1/kernelApplicationProcess/osApplicationProcess/ipcProcesses/ipcProcessID=5";
	...
	protected static void handleException(Throwable e) {
		...
		System.out.println("Aquesta és una unhandled exception");
	}
	...
	public void run(){
        ...
        while (true)
        {
			try
			{
				...
				int bytes_read = rina.getIpcManager().readSDU(flow_.getPortId(), buffer, Manager.max_sdu_size_in_bytes);
			...
			catch(FlowNotAllocatedException e)
			{
				System.out.println("FlowNotAllocatedException in cacep: " + e.getMessage());
				break;
			}
        }
	...
	private boolean createIPCP_2(int port_id)
	{
		byte[] buffer = new byte[max_sdu_size_in_bytes];
		...
		ApplicationProcessNamingInformation proc_name = new ApplicationProcessNamingInformation();
		proc_name.setProcessInstance("1");;
		proc_name.setProcessName("test2.IRATI");
		...
		// Neighbors to enroll
		neighbor_config_t neigh =  new neighbor_config_t();
		ApplicationProcessNamingInformation dif = new ApplicationProcessNamingInformation();
		dif.setProcessName("NMS.DIF");
		neigh.setDif(dif);
		ApplicationProcessNamingInformation using_dif = new ApplicationProcessNamingInformation();
		using_dif.setProcessName("300");
		neigh.setUnder_dif(using_dif);
		ApplicationProcessNamingInformation ipcp_name = new ApplicationProcessNamingInformation();
		ipcp_name.setProcessName("test1.IRATI");
		neigh.setNeighbor_name(ipcp_name);
